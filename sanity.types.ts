...
// Source: sanity/lib/queries.ts
// Variable: boardImageBySkuQuery
// Query: *[_type == "board" && skuCode == $sku && !(_id in path("drafts.**"))] | order(select(language == $locale => 0, 1) asc) [0] {    "name": name,    mainImage,    "aspectRatio": mainImage.asset->metadata.dimensions.aspectRatio  }
export type BoardImageBySkuQueryResult = {
  name: string;
  mainImage: {
    asset?: SanityImageAssetReference;
    media?: unknown;
...
// Source: sanity/lib/queries.ts
// Variable: productImagesBySkuQuery
// Query: *[_type in ["board", "accessory"] && skuCode in $skus && !(_id in path("drafts.**"))] | order(select(language == $locale => 0, 1) asc) {    skuCode,    // Only what urlFor reads. crop/hotspot look unused — no image sets them today —    // but both schemas enable hotspot, so dropping them would silently ignore an    // editor's framing the first time one is applied. alt is left out on purpose:    // every call site prints the product name beside the picture.    mainImage { asset, crop, hotspot }  }
export type ProductImagesBySkuQueryResult = Array<
  | {
      skuCode: string;
      mainImage: {
        asset: SanityImageAssetReference | null;
...
// Source: sanity/lib/queries.ts
// Variable: companySettingsQuery
// Query: *[_type == "companySettings"][0] { legalName, taxId, address, email, logo }
export type CompanySettingsQueryResult = {
  legalName: string;
  taxId: string;
  address: string | null;
  email: string | null;
...
declare module "@sanity/client" {
  interface SanityQueries {
    ...
    '\n  *[_type == "board" && skuCode == $sku && !(_id in path("drafts.**"))] | order(select(language == $locale => 0, 1) asc) [0] {\n    "name": name,\n    mainImage,\n    "aspectRatio": mainImage.asset->metadata.dimensions.aspectRatio\n  }\n': BoardImageBySkuQueryResult;
