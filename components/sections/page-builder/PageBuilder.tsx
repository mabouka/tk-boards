...
import SectionAboutPreview from '@/components/sections/about-preview/SectionAboutPreview'
import BoardsPreviewClient from '@/components/sections/boards-preview/BoardsPreviewClient'
...
import SectionCenteredText from '@/components/sections/centered-text/SectionCenteredText'
import SectionTextYoutube from '@/components/sections/text-youtube/SectionTextYoutube'
import WorkshopSection from '@/components/contact/WorkshopSection'
import type { FeatureItem } from '@/components/sections/features/SectionFeatures'
import type { OutlineMilestone } from '@/components/sections/outline/SectionOutline'
import type { FixedImageItem } from '@/components/sections/fixed-image/SectionFixedImage'
import type { ScrollImageItem } from '@/components/sections/scroll-image/SectionScrollImage'

// Lazy-loaded so GSAP only ships on pages that actually render these sections.
const SectionFeatures = dynamic(() => import('@/components/sections/features/SectionFeatures'))
const SectionOutline = dynamic(() => import('@/components/sections/outline/SectionOutline'))
const SectionMarquee = dynamic(() => import('@/components/sections/marquee/SectionMarquee'))
const SectionBigScrollText = dynamic(
  () => import('@/components/sections/big-scroll-text/SectionBigScrollText')
)
const SectionScrollImage = dynamic(
  () => import('@/components/sections/scroll-image/SectionScrollImage')
)
    ...
    <>
      {sections.map((section) => {
        switch (section._type) {
          ...
          case 'sectionBigScrollText':
            return section.text ? (
              <SectionBigScrollText key={section._key} text={section.text} />
            ) : null
          case 'sectionTextYoutube':
            return (
              <SectionTextYoutube
                key={section._key}
                label={section.label ?? undefined}
                ...
                ctas={section.ctas?.map(toCta)}
                theme={section.theme ?? undefined}
                videoPosition={section.videoPosition ?? undefined}
              />
            )
          case 'sectionScrollImage': {
            const scrollItems: ScrollImageItem[] = (section.scrollImages ?? []).map((it) => ({
              ...
              widthColumns: it.widthColumns ?? 6,
              offsetY: it.offsetY ?? 0,
              speed: it.speed ?? 0.4,
            }))
            return <SectionScrollImage key={section._key} items={scrollItems} />
          }
          default:
            return null
        }
